...
When new packages are used, `install.packages()` does not install packages globally, it does in an environment only used for our project. You can find this library in `renv/library`.
...
If a developer adds a new package to a project, `renv::snapshot` updates the lockfile which triggers an update cascade, once committed and pushed to GitHub.
